...
class DeepSort(object):
    ...
    """
    @staticmethod
    def _xywh_to_tlwh(xywh):
        """Convert [x_center, y_center, width, height] to [top_left_x, top_left_y, width, height]."""
        if xywh.shape[0] == 0:
            return xywh.clone()
        ...
        tlwh = xywh.clone()
        tlwh[:, 0] = xywh[:, 0] - xywh[:, 2] / 2.0  # top_left_x = x_center - width / 2
        tlwh[:, 1] = xywh[:, 1] - xywh[:, 3] / 2.0  # top_left_y = y_center - height / 2
