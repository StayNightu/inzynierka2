...
class Sledzenie:
    ...
    def draw_boxes(self, img, bbox, identities=None, offset=(0, 0)):
        ...
        for i, box in enumerate(bbox):
            ...
            cv2.rectangle(overlay, (x1, y1), (x2, y2), color=self.compute_color_for_labels(id), thickness=2,
                          lineType=cv2.LINE_AA)
            label = str(id)
            t_size = cv2.getTextSize(str(label), cv2.FONT_HERSHEY_SIMPLEX, fontScale=1 / 2, thickness=1)[0]
            c2 = x1 + t_size[0], y1 - t_size[1] - 3
            cv2.rectangle(overlay, (x1, y1), c2, color=self.compute_color_for_labels(id), thickness=-1,
                          lineType=cv2.LINE_AA)
            cv2.putText(overlay, str(label), (x1, y1 - 2), 0, 1 / 2, [255, 255, 255], thickness=1, lineType=cv2.LINE_AA)
        combined_img = cv2.addWeighted(img, 1, overlay, 0.5, 0)
        return combined_img

    def process_video(self):
        unique_id_counter = 0
        with open('./bbox_data.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['detection_id', 'frame_number', 'x_top_left', 'y_top_left', 'width', 'height', 'bb_id'])
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            self.out = cv2.VideoWriter('output.avi', fourcc, 33.0, (1280, 720))
            prev_frame = None
            frame_number = 0
            while True:
                xywh_bboxs = []
                    ...
                    if prev_frame is not None:
                        ...
                        if len(contours) > 0:
                            ...
                            if len(outputs) > 0:
                                ...
                                self.draw_boxes(frame, bbox_xyxy, identities)
                    self.out.write(frame)
                    for bbox_data in outputs:
                        x1, y1, x2, y2, identity = bbox_data[:5]
                        bbox_width = x2 - x1
                        bbox_height = y2 - y1
                        identity_formatted = f"id:{identity}"
                        if identity < len(confs):
                            writer.writerow(
                                [unique_id_counter, frame_number, x1, y1, bbox_width, bbox_height, identity_formatted])
                            unique_id_counter += 1
                        else:
                            print(f"Invalid identity index: {identity}")
                    frame_number += 1
